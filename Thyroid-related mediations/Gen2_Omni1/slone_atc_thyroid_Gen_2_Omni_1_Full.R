...
joined_data_08_1b <- left_join(cod_08_1b, thyroid_a_s_replace, by = c("medname", "atc_cod1", "atc_cod2"))
...
joined_data_08_1b$medname <- coalesce(joined_data_08_1b$New_medname, joined_data_08_1b$medname)
joined_data_08_1b$atc_cod1 <- coalesce(joined_data_08_1b$New_atc_cod1, joined_data_08_1b$atc_cod1)
joined_data_08_1b$atc_cod2 <- coalesce(joined_data_08_1b$New_atc_cod2, joined_data_08_1b$atc_cod2)

#Select the updated columns along with the necessary unchanged columns
cod_08_1b_updated <- joined_data_08_1b[,c("id", "idtype", "framid", "medname", "atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4")]

#Select the rows of individual ATC data containing the specific ATC numbers in the lists for drugs of this type
thyroid_filtered_08_1b <- cod_08_1b_updated %>% filter_all(any_vars(. %in% thyroid_atc_list))
...
thyroid_08_1b_join_multiple <- inner_join(thyroid_multiple_08_1b, thyroid_a_s_multiple, by = c("atc_cod1", "atc_cod2"))
...
#Join old and replacement
joined_data_09_1b <- left_join(cod_09_1b, thyroid_a_s_replace, by = c("medname", "atc_cod1", "atc_cod2"))

#Update certain columns by using coalesce function. Where the new column values exist, they replace the old ones
#This also leaves the ones we don't want replaced alone
joined_data_09_1b$medname <- coalesce(joined_data_09_1b$New_medname, joined_data_09_1b$medname)
joined_data_09_1b$atc_cod1 <- coalesce(joined_data_09_1b$New_atc_cod1, joined_data_09_1b$atc_cod1)
joined_data_09_1b$atc_cod2 <- coalesce(joined_data_09_1b$New_atc_cod2, joined_data_09_1b$atc_cod2)

#Select the updated columns along with the necessary unchanged columns
cod_09_1b_updated <- joined_data_09_1b[,c("id", "idtype", "framid", "medname", "atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4")]

#Select the rows of individual ATC data containing the specific ATC numbers in the lists for drugs of this type
thyroid_filtered_09_1b <- cod_09_1b_updated %>% filter_all(any_vars(. %in% thyroid_atc_list))
...
#Match multiple ATCs to one Slone
thyroid_09_1b_join_multiple <- inner_join(thyroid_multiple_09_1b, thyroid_a_s_multiple, by = c("atc_cod1", "atc_cod2"))
...
#Join old and replacement
joined_data_10_1b <- left_join(cod_10_1b, thyroid_a_s_replace, by = c("medname", "atc_cod1", "atc_cod2"))

#Update certain columns by using coalesce function. Where the new column values exist, they replace the old ones
#This also leaves the ones we don't want replaced alone
joined_data_10_1b$medname <- coalesce(joined_data_10_1b$New_medname, joined_data_10_1b$medname)
joined_data_10_1b$atc_cod1 <- coalesce(joined_data_10_1b$New_atc_cod1, joined_data_10_1b$atc_cod1)
joined_data_10_1b$atc_cod2 <- coalesce(joined_data_10_1b$New_atc_cod2, joined_data_10_1b$atc_cod2)

#Select the updated columns along with the necessary unchanged columns
cod_10_1b_updated <- joined_data_10_1b[,c("id", "idtype", "framid", "medname", "atc_cod1", "atc_cod2", "atc_cod3")]

#Select the rows of individual ATC data containing the specific ATC numbers in the lists for drugs of this type
thyroid_filtered_10_1b <- cod_10_1b_updated %>% filter_all(any_vars(. %in% thyroid_atc_list))
...
#Match multiple ATCs to one Slone
thyroid_10_1b_join_multiple <- inner_join(thyroid_multiple_10_1b, thyroid_a_s_multiple, by = c("atc_cod1", "atc_cod2"))
