# ******************************************************************************************************************************************
# Introduction to Gen 3/Omni 2/NOS Exam 1-3 Steroids drug usage ATC/Slone conversion source code
# ******************************************************************************************************************************************
#   
...
# 
# 
# The purpose of this R code is to convert the steroid ATC codes in the Gen 3/Omni 2/NOS Exam 3 Exam 1-3 ATC datasets
...
#Join ATC and Slone
steroid_01_3b_joined <- inner_join(cod_01_3b, steroid_a_s, by = c("medname", "atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4"))

#remove older atc columns
steroid_01_3b_corrected <- steroid_01_3b_joined[ , !(names(steroid_01_3b_joined) %in% c("atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4"))]

#rename to correct columns to just atc
colnames(steroid_01_3b_corrected)[5:8] <- c("atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4")

#reorder rows
steroid_01_3b_ordered <- steroid_01_3b_corrected %>% arrange(framid)

#add exam number
steroid_01_3b_ordered$exam_num <- 1
steroid_01_3b_final <- steroid_01_3b_ordered %>% relocate(exam_num, .before = id)
...
#Join ATC and Slone
steroid_02_3b_joined <- inner_join(cod_02_3b, steroid_a_s, by = c("medname", "atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4"))

#remove older atc columns
steroid_02_3b_corrected <- steroid_02_3b_joined[ , !(names(steroid_02_3b_joined) %in% c("atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4"))]

#rename to correct columns to just atc
colnames(steroid_02_3b_corrected)[5:8] <- c("atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4")

#reorder rows
steroid_02_3b_ordered <- steroid_02_3b_corrected %>% arrange(framid)

#add exam number
steroid_02_3b_ordered$exam_num <- 2
steroid_02_3b_final <- steroid_02_3b_ordered %>% relocate(exam_num, .before = id)
...
#Join ATC and Slone
steroid_03_3b_joined <- inner_join(cod_03_3b, steroid_a_s_4_removed, by = c("medname", "atc_cod1", "atc_cod2", "atc_cod3"))

#remove older atc columns
steroid_03_3b_corrected <- steroid_03_3b_joined[ , !(names(steroid_03_3b_joined) %in% c("atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4"))]

#rename to correct columns to just atc
colnames(steroid_03_3b_corrected)[5:8] <- c("atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4")

#reorder rows
steroid_03_3b_ordered <- steroid_03_3b_corrected %>% arrange(framid)

#add exam number
steroid_03_3b_ordered$exam_num <- 3
steroid_03_3b_final <- steroid_03_3b_ordered %>% relocate(exam_num, .before = id)


#### Final Section ####

#bind data frames for each exam
steroid_full_final <- bind_rows(steroid_01_3b_final, steroid_02_3b_final, steroid_03_3b_final)
#write final CSV to file
write.csv(steroid_full_final, file = "Slone_ATC_Steroid_Gen_3_Omni_2_NOS_Full.csv", row.names = FALSE)
